...
def MAPE_np(pred, true, mask_value=None):
    if mask_value != None:
        mask_qe = np.where(((pred==mask_value) + (true==mask_value))==2, True, False)
        mask = np.where(true > (mask_value), True, False)
        true = true[mask]
        pred = pred[mask]

    return np.sum(np.absolute(np.divide((true - pred), true)))/np.sum([mask,mask_qe])
# def MAPE_np(y_pred, y_true, null_val=np.nan):
...
if __name__ == '__main__':
    pred = np.random.randn(32, 1, 12,207)
    true = np.random.randn(32, 1, 12,207)
    print(MAPE_np(pred, true, 0))
