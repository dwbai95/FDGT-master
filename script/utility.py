...
def calculate_laplacian_matrix(adj_mat, mat_type):
    ...
    # D = D_row as default
    deg_mat = deg_mat_row  # deg
    adj_mat = np.asmatrix(adj_mat)

    deg_mat_inv = np.linalg.inv(deg_mat)
    deg_mat_inv[np.isinf(deg_mat_inv)] = 0.0  #

    deg_mat_inv_sqrt = fractional_matrix_power(deg_mat, -0.5)
    deg_mat_inv_sqrt[np.isinf(deg_mat_inv_sqrt)] = 0.0
    ...
    wid_deg_mat_inv[np.isinf(wid_deg_mat_inv)] = 0.0

    wid_deg_mat_inv_sqrt = fractional_matrix_power(wid_deg_mat, -0.5)
    wid_deg_mat_inv_sqrt[np.isinf(wid_deg_mat_inv_sqrt)] = 0.0
    ...
    sym_normd_lap_mat = id_mat - np.matmul(
        np.matmul(deg_mat_inv_sqrt, adj_mat), deg_mat_inv_sqrt
    )
    ...
    hat_sym_normd_lap_mat = np.matmul(
        np.matmul(wid_deg_mat_inv_sqrt, wid_adj_mat), wid_deg_mat_inv_sqrt
    )
...
def evaluate_metric(model, data_iter, zscore, n_pred, device):
    ...
    with torch.no_grad():
        ...
        for x, y in data_iter:
            ...
            out_unnormalized = zscore.inverse_transform(y_pred)
        ...
        out_unnormalized[out_unnormalized < 0] = 0
        ...
        corr = []
        for i in range(12):
            MAE, MAPE, RMSE, RRSE, CORR = metrics.All_Metrics(
                pred=out_unnormalized[i],
                true=target_unnormalized[i],
                mask1=None,
                mask2=0.0,
            )
            mae.append(MAE)
        ...
        test_MAE, test_MAPE, test_RMSE, test_RRSE, test_CORR = metrics.All_Metrics(
            pred=out_unnormalized, true=target_unnormalized, mask1=None, mask2=0.0
        )

        print(
            "Average Horizon, MAE: {:.2f}, MAPE: {:.2f}%, RMSE: {:.2f}".format(
        ...
        accuracy_pd = pd.DataFrame(
            accuracy, index=accuracy_index, columns=accuracy_columns
        )
        print(accuracy_pd)
